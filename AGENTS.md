...
Then read [`__agent__/skills/dsh-scaffold/SKILL.md`](__agent__/skills/dsh-scaffold/SKILL.md) (and [`__agent__/skills/olares-cli-setup/SKILL.md`](__agent__/skills/olares-cli-setup/SKILL.md) when the CLI is missing). Follow [`__agent__/rules/dsh-scaffold.md`](__agent__/rules/dsh-scaffold.md) every turn. After install, continue the user’s request. First-user init: `npm run preflight` (Node 22+, logged-in `olares-cli`, **their** GitHub fork as origin, logged-in `gh`). Images are built in GitHub Actions. Never run `olares-cli profile login` or `gh auth login`. After the first image build for a name, they set that GHCR package Public in GitHub.
